   ...
   try:
    ...
    if message.new_chat_members[0].username == "COUDRA_1":
      try:
         ...
         await client.set_administrator_title(chat_id, user_id, "كودرا")
      except:
        pass
      return await message.reply_text(f"**🎸 انضم المطو كودرا للشات  💎 .\n🎸 مرحبا بك : @COUDRA_1  💎 .**")
    dev = await get_dev(bot_username)
    ...
    if message.new_chat_members[0].id == bot.id:
      ...
      button = [
[InlineKeyboardButton(text="✧ 𝖲𝗈𝗎𝗋𝖼𝖾 ✧", url=f"{ch}"),InlineKeyboardButton(text="✧ 𝖦𝗋𝗈𝗎𝗉  ✧", url=f"{gr}")],
...
"مره واحد اسمو كودرا  كان بيرجم ابليس ف الحج قالولو ليه؟ قال عشان يمكن احتاجو 👻😹",
...
"لا كودرا على الدوق فيما يحب أو بكره",
...
@Client.on_message(filters.command(["مطور السورس","كودرا","المبرمج"], ""))
...
async def set_vi_so(client: Client, message):
   NAME = await client.ask(message.chat.id,"**🎸 ارسل رابط لوجو السورس  💬\n🎸 مثال ⟨ https://telegra.ph/file/09b8ec504336aada15ab7.png ⟩  💎 .**", filters=filters.text, timeout=30)
   VID_SO = NAME.text
   bot_username = client.me.username
...
async def tiillli(client, message):
  if message.from_user.username in ["COUDRA_1"]:
   ...
   if message.from_user.username == "COUDRA_1":
     await message.reply_text(f"**🎸 تم تنفيذ الامر بواسطة المطور كودرا  💎 .**")
   else:
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="**🎸 𝑱𝒐𝒊𝒏 ➧ @WU_SD  💎 .**")
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**🎸 𝑱𝒐𝒊𝒏 ➧ @WU_SD  💎 .**")
...
async def videoo(client, message):
  ...
  await message.reply_video(video=video, caption="**🎸 𝑱𝒐𝒊𝒏 ➧ @WU_SD  💎 .**")
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in OWNER :
         ...
         return
     if userr.username in ["WU_SD"]:
         await message.reply_text("**🎸 رتبتك هي : المطور كودرا  ❤**")
         return
     if userr.username in ["WU_SD"]:
         await message.reply_text("**🎸 رتبتك هي : المطور كودرا ❤**")
         return
...
async def wel__come(client: Client, message):
	...
	await client.send_message(text=f"✪ لا تسئ اللفظ وان ضاق عليك الرد\n✧ 𝘋𝘦𝘷 𝘚𝘰𝘶𝘳𝘤𝘦 ✧ » {message.from_user.mention}\n✧ 𝖦𝗋𝗈𝗎𝗉 ✧» {message.chat.title}",chat_id=chatid)
